...
async function useRedisAuthState() {
    const credsData = await redis.get('wa:creds');
    const creds = credsData ? JSON.parse(credsData, BufferJSON.reviver) : initAuthCreds();

    const state = {
        creds,
        keys: {
            get: async (type, ids) => {
                const data = await redis.get(`wa:keys:${type}`);
                const parsed = data ? JSON.parse(data, BufferJSON.reviver) : {};
                return ids.reduce((dict, id) => {
                    if (id in parsed) dict[id] = parsed[id];
                    return dict;
                }, {});
            },
            set: async (data) => {
                for (const type of Object.keys(data)) {
                    const existing = await redis.get(`wa:keys:${type}`);
                    const parsed = existing ? JSON.parse(existing, BufferJSON.reviver) : {};
                    for (const id in data[type]) parsed[id] = data[type][id];
                    await redis.set(`wa:keys:${type}`, JSON.stringify(parsed, BufferJSON.replacer));
                }
            }
        }
    };
    ...
    async function saveCreds() {
        await redis.set('wa:creds', JSON.stringify(state.creds, BufferJSON.replacer));
    }
...
async function startBot() {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            schedule.scheduleJob({ rule: '00 20 * * 1,3,5,6,0', tz: 'Asia/Yekaterinburg' }, async () => {
                // const chatId = '79519388508@s.whatsapp.net'
                const chatId = '79082621560@s.whatsapp.net'
